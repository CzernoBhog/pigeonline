...
class Utility implements \JsonSerializable
{
    ...
    public static function createWhere(array $params, String $table, BOOL $orClause = FALSE, BOOL $replaceWithLIKE = FALSE, String $orderBy = NULL, array $joinTablesWithOnColumns = null, $tableJoinColumn = null, String $select = '*', $joinType = "INNER", $limit = null)
    {
        ...
        if (!is_null($joinTablesWithOnColumns)) {
            if (is_array($tableJoinColumn)) {
                $keys = array_keys($joinTablesWithOnColumns);
                $values = array_values($joinTablesWithOnColumns);
                for ($i = 0; $i < count($joinTablesWithOnColumns); $i++) {
                    $query .= " $joinType JOIN $keys[$i] ON($table.$tableJoinColumn[$i] = $keys[$i].$values[$i])";
                }
            } else {
                foreach ($joinTablesWithOnColumns as $tableToJoin => $column) {
                    if (!is_null($column)) {
                        $query .= " $joinType JOIN $tableToJoin ON($table.$tableJoinColumn = $tableToJoin.$column)";
                    }
                }
            }
        }
